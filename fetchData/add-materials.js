...
async function addMaterialsFromCSV() {
  ...
  try {
    ...
    // 讀取 CSV 檔案
    const csvPath = process.env.CSV_PATH || path.join(__dirname, "materials.csv");
    ...
    for (let i = 0; i < records.length; i++) {
      ...
      try {
        ...
        // 料號
        if (record.料號) {
          await page.fill('input[name="materialCode"], input[placeholder*="料號"]', record.料號);
        }

        // 品名
        if (record.品名) {
          await page.fill('input[name="productName"], input[placeholder*="品名"]', record.品名);
        }

        // 申請人
        if (record.申請人) {
          await page.fill('input[name="applicant"], input[placeholder*="申請人"]', record.申請人);
        }

        // 部門
        if (record.部門) {
          await page.fill('input[name="department"], input[placeholder*="部門"]', record.部門);
        }

        // 物料類別（下拉選單）
        if (record.物料類別) {
          await page.selectOption('select[name="materialType"], select:has-text("請選擇")', record.物料類別);
        }

        // 規格
        if (record.規格) {
          await page.fill('textarea[name="specification"], textarea[placeholder*="規格"]', record.規格);
        }

        // 數量
        if (record.數量) {
          await page.fill('input[name="quantity"], input[placeholder*="數量"]', record.數量);
        }
